...
bool GameApp::InitResource()
{
	...
	{			
		//当然这里也可以用D3D11_PRIMITIVE_TOPOLOGY_LINESTRIP 按一系列线段进行装配，每相邻两个顶点(或索引数组相邻的两个索引对应的顶点)构成一条线段
		...
		//相应m_pd3dImmediateContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_LINESTRIP);
		//m_pd3dImmediateContext->Draw(7, 0);

		//这里利用的是D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST	按一系列三角形进行装配，每三个顶点(或索引数组每三个索引对应的顶点)构成一个三角形
		//有了6个三角形，空间可能大了点，不过渲染颜色更好看些。
